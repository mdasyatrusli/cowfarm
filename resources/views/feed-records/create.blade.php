            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('feed-records.store') }}">
                        ...
                        <div class="mb-4">
                            <x-input-label for="cow_id" :value="__('Sapi')" />
                            <select id="cow_id" name="cow_id" class="block mt-1 w-full border-gray-300 focus:border-farm-green focus:ring-farm-green rounded-md shadow-sm" required>
                                <option value="">{{ __('Pilih sapi...') }}</option>
                                @foreach ($cows as $cow)
                        ...
                        <div class="mb-4">
                            <x-input-label for="record_date" :value="__('Tanggal')" />
                            <x-text-input id="record_date" class="block mt-1 w-full focus:border-farm-green focus:ring-farm-green" type="date" name="record_date" :value="old('record_date', date('Y-m-d'))" required />
