        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('milk-records.store') }}" method="POST">
                        @csrf
                        ...
                        <!-- Session -->
                        <div class="mb-4">
                            <label for="session" class="block text-sm font-medium text-gray-700">{{ __('Sesi Pemerahan') }}</label>
                            <select name="session" id="session" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-farm-green focus:ring-farm-green">
                                <option value="pagi" {{ old('session') == 'pagi' ? 'selected' : '' }}>{{ __('Morning') }}</option>
                                <option value="sore" {{ old('session') == 'sore' ? 'selected' : '' }}>{{ __('Afternoon') }}</option>
                            </select>
                            @error('session')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" min="0" name="volume_liters" id="volume_liters" value="{{ old('volume_liters') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-farm-green focus:ring-farm-green">
                            @error('volume_liters')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
